...
uploaded = st.sidebar.file_uploader(
    "Upload CSV (same schema as your notebooks, or Online Retail for Segmentation/Recs)",
    type=["csv"],
)
...
# Derive Engaged flag from Response (1 if Response == Yes else 0)
...
# -----------------------------------------------------------------------------
# Page: Why People Engage  (UPDATED earlier)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Page: Product Recommendations – Online Retail dataset
# -----------------------------------------------------------------------------
elif page == "Product Recommendations":
    st.markdown("## Product Recommendations")

    st.info(
        "This view uses the **Online Retail** dataset (UCI) to surface simple, "
        "transparent **“people who bought X also bought Y”** recommendations.\n\n"
        "To use it, upload the Online Retail CSV (with columns like "
        "`InvoiceNo`, `StockCode`, `Description`, `Quantity`, `UnitPrice`, `CustomerID`)."
    )

    retail_required_recs = [
        "InvoiceNo",
        "StockCode",
        "Description",
        "Quantity",
    ...
    if not all(c in df.columns for c in retail_required_recs):
        st.warning(
            "To use this page, upload the Online Retail dataset (or a dataset with at "
            "least these columns): "
            + ", ".join(retail_required_recs)
        )
    else:
        retail = df.copy()
        retail = retail.dropna(subset=["InvoiceNo", "StockCode", "Description", "CustomerID"])

        # Ensure numeric
        retail["Quantity"] = pd.to_numeric(retail["Quantity"], errors="coerce").fillna(0)
        retail["UnitPrice"] = pd.to_numeric(retail["UnitPrice"], errors="coerce").fillna(0)
        ...
        if top_products.empty:
            ...
            base_product = st.selectbox(
                "Pick a product as the anchor for recommendations:",
                options=top_products["Description"],
            )

            base_invoices = retail.loc[
                retail["Description"] == base_product, "InvoiceNo"
            ].unique()

            co_df = retail[retail["InvoiceNo"].isin(base_invoices)]
            co_df = co_df[co_df["Description"] != base_product]

            if co_df.empty:
                st.warning("No co-purchases found for this product in the current sample.")
            else:
                co_counts = (
...
- Each row shows a product that appears on the same invoices as your base product.  
- Use this to design **bundles**, **“complete the basket”** recommendations, or targeted cross-sell journeys.  
- Because it’s based on invoice co-occurrence, it’s a simple and explainable starting point before moving to more complex models (e.g., matrix factorization or deep learning).  
"""
                )
